...
import { getSmsSettings, updateSmsSettings, sendTestSms } from "@/lib/superadmin.functions";
              ...
              <header className="flex items-start gap-3 border-b border-border p-5">
                ...
                <div className="flex-1">
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">
                    ...
                    mode, the booking flow returns the code in the response so you can test without
                    a paid SMS account.
                  </p>
                </div>
              ...
              <div className="space-y-5 p-5">
                <div className="grid gap-2 sm:max-w-xs">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Recommended: On-screen code — free, no SMS costs. SMS providers are available
                    for future use.
                  </p>
                  <Select
                    value={provider}
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="on_screen">
                        On-screen code — free, no SMS, browser-session bound
                      </SelectItem>
                  ...
                  {provider === "on_screen" ? (
                    <p className="text-xs text-muted-foreground">
                      On-screen codes are free and require no SMS provider. The 6-digit code is
                      shown directly in the patient's browser, expires in 60 seconds, and is bound
                      to that browser tab. Existing SMS credentials are preserved below.
                    </p>
                  ) : (
                    <p className="text-xs text-muted-foreground">
                      Existing secrets are masked. Leave a field blank to keep its current value.
                    </p>
                  )}
                </div>
                ...
                {provider === "twilio" && (
                  <div className="grid gap-3 sm:grid-cols-2">
                    <Field
                      ...
                      onChange={(v) => {
                        setForm({ ...form, twilio: { ...form.twilio, accountSid: v } });
                        mark("twilio.accountSid");
                      }}
                    ...
                    <Field
                      ...
                      onChange={(v) => {
                        setForm({ ...form, twilio: { ...form.twilio, authToken: v } });
                        mark("twilio.authToken");
                      }}
                      placeholder="••••••••"
                    />
                    <Field
                      ...
                      onChange={(v) => {
                        setForm({ ...form, twilio: { ...form.twilio, fromNumber: v } });
                        mark("twilio.fromNumber");
                      }}
                ...
                {provider === "msg91" && (
                  <div className="grid gap-3 sm:grid-cols-2">
                    <Field
                      ...
                      onChange={(v) => {
                        setForm({ ...form, msg91: { ...form.msg91, authKey: v } });
                        mark("msg91.authKey");
                      }}
                      placeholder="••••••••"
                    />
                    <Field
                      ...
                      onChange={(v) => {
                        setForm({ ...form, msg91: { ...form.msg91, senderId: v } });
                        mark("msg91.senderId");
                      }}
                      placeholder="CLINIC"
                    />
                    <Field
                      ...
                      onChange={(v) => {
                        setForm({ ...form, msg91: { ...form.msg91, templateId: v } });
                        mark("msg91.templateId");
                      }}
                ...
                {provider === "gupshup" && (
                  <div className="grid gap-3 sm:grid-cols-2">
                    <Field
                      ...
                      onChange={(v) => {
                        setForm({ ...form, gupshup: { ...form.gupshup, apiKey: v } });
                        mark("gupshup.apiKey");
                      }}
                      placeholder="••••••••"
                    />
                    <Field
                      ...
                      onChange={(v) => {
                        setForm({ ...form, gupshup: { ...form.gupshup, source: v } });
                        mark("gupshup.source");
                      }}
                    ...
                    <Field
                      ...
                      onChange={(v) => {
                        setForm({ ...form, gupshup: { ...form.gupshup, appName: v } });
                        mark("gupshup.appName");
                      }}
                    />
                  </div>
                )}
                ...
                {provider === "dev" && (
                  <div className="rounded-lg border border-dashed border-border bg-muted/40 p-4 text-sm text-muted-foreground">
                    Dev mode is active. OTP codes are returned in the booking response and never
                    sent over SMS. Switch to a real provider before launch.
                  </div>
                )}
              </div>
